...
class ActionType(Enum):
    ...
    # 1D (identical input to all motors) with desired thrust and torques
    ONE_D_DYN = "one_d_dyn"
    # 1D (identical input to all motors) with PID control
    ONE_D_PID = "one_d_pid"
...
class BaseSingleAgentAviary(BaseAviary):
    ...
    def __init__(self,
        ...
        dynamics_attributes = True if act in [
            ActionType.DYN, ActionType.ONE_D_DYN] else False
        self.OBS_TYPE = obs
        self.ACT_TYPE = act
        self.REW_TYPE = rew
    ...
    def _preprocessAction(self,
        ...
        if self.ACT_TYPE == ActionType.TUN:
            self.ctrl.setPIDCoefficients(p_coeff_pos=(action[0]+1)*self.TUNED_P_POS,
                                         i_coeff_pos=(
                                             action[1]+1)*self.TUNED_I_POS,
                                         d_coeff_pos=(
                                             action[2]+1)*self.TUNED_D_POS,
                                         p_coeff_att=(
                                             action[3]+1)*self.TUNED_P_ATT,
                                         i_coeff_att=(
                                             action[4]+1)*self.TUNED_I_ATT,
                                         d_coeff_att=(
                                             action[5]+1)*self.TUNED_D_ATT
                                         )
