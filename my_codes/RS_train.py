...
def fit_dynamics(n_iter=50):
    ...
    for _ in range(n_iter):
        ...
        next_pos, next_ang, next_vel, next_gyr = parse_obses(
            samples["next_obs"])
        inputs = np.concatenate([ang, gyr, samples["act"]], axis=1)
        labels = np.concatenate([next_ang - ang, next_gyr - gyr], axis=1)
...
if __name__ == "__main__":
    ...
    filename = datetime.datetime.now().strftime('%m%d_%H:%M:%S') + '.pt'
    torch.save({
